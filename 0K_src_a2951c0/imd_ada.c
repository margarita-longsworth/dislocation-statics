...
void init_ada(void) {
	...
	if (ada_nbr_r2cut == 0.){
		if (ada_crystal_structure == ADA_FCC_CONFIG) {
			...
			/*cutoff = ~1.22*nearest neighbor dist*/
			ada_nbr_r2cut = SQR(0.862*ada_latticeConst);
		} else if (ada_crystal_structure == ADA_NITI_CONFIG) {
			/*cutoff = ~1.22*second nearest neighbor dist*/
			ada_nbr_r2cut = SQR(1.22*ada_latticeConst);
		} else if (ada_crystal_structure == ADA_FEC_CONFIG) {
			/*cutoff = ~1.22*second nearest neighbor dist*/
			ada_nbr_r2cut = SQR(1.22*ada_latticeConst);
		}
	}
	...
	if (myid == 0){
		if (ada_crystal_structure == ADA_FCC_CONFIG) {
			...
			printf( "ADA: FeC analysis scheme selected \n");
		} else if (ada_crystal_structure == ADA_NITI_CONFIG) {
			printf( "ADA: NiTi analysis scheme selected \n");
		} else {
			error("Crystal structure not supported by ADA");
		}
		...
		for (k = 0; k < ncells; k++) {
			...
			for (i = 0; i < p->n; i++) {
				...
				for (j = 0; j < nneigh; j++) {
					...
					ii = NEIGH(p, i)->num[j];
					if (SORTE(q,ii) == 0) ++count;
				}

				if (SORTE(p,i) == 1) type = 7;
				else if (count < 11) type = 6;
				else if (count == 11) type = 4;
				else if (count >= 15) type = 5;
					...
					if (co_x0 > 5 && co_x0+co_x1==7 && co_x2<=2 && count==14)
						type = 0;
					else if (count==13 && nneigh==14 && co_x0==6)
						type = 0;
					else if (count == 13)
						type = 4;
					else if (count == 12)
			...
			for (i = 0; i < p->n; i++) {
				...
				if (nneigh <= 10) type = 6;
					...
					for (j = 0; j < nneigh; j++) {
						...
						if (SORTE(p,i) == SORTE(q,ii)) num_same++;
						else num_other++;

						for (jj = 0; jj < j; jj++) {
							q = NEIGH(p, i)->cl[jj];
							ii = NEIGH(p, i)->num[jj];
							if (type_i == SORTE(q,ii)){
								real *tmp_ptr  = &NEIGH(p,i)->dist[3*jj];
								v2.x = *tmp_ptr; tmp_ptr++;
								v2.y = *tmp_ptr; tmp_ptr++;
								v2.z = *tmp_ptr;
								...
								int same = SORTE(p,i) == SORTE(q,ii);
								if (same){
									if (angle < -.95) co_x0_same++;
								} else {
									if (angle < -.95) co_x0_other++;
									else if (angle < -.90) co_x1_other++;
									else if (angle < -.60) co_x2_other++;
									else if (angle < -.30) co_x3_other++;
									else if (angle < -.20) co_x4_other++;
									else if (angle < .20) co_x5_other++;
									else if (angle < 0.45) co_x6_other++;
									else if (angle < 0.7) co_x7_other++;
								}
							}
						}
					}
					if (num_same==6 && num_other == 8 && co_x0_other == 2 && co_x0_same == 1 && co_x3_other+co_x4_other>=10) type = 5; //APB
					else if ( SORTE(p,i) == 0 && num_same >= 5 && num_other>=7 && co_x5_other+co_x7_other > 3 && co_x3_other<12 && co_x6_other<12) type = 3;  //MartTi
					else if ( SORTE(p,i) == 1 && num_same >= 5 && num_other>=7 && co_x3_other <= co_x4_other+co_x5_other && co_x6_other<12) type = 3;  //MartNi
					else if (num_same == 6 && num_other>=num_same && (co_x3_other >= 7 || (co_x3_other == 6 && co_x6_other==12)) && co_x6_other > 9 && co_x0_other >= 3 && co_x0_same >= 2) type = 0;  //B2
					else type = 4;
				}
				ADATYPE(p, i) = type;
			}
