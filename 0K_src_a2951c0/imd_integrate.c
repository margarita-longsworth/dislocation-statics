...
#endif
#ifdef BER
  real cc, cc_ph; /* cc_ph - prefactor for placeholder system*/
#endif
#ifdef BER /* reproduce Ju Li's implementation of the Berendsen Thermostat */
#ifdef BERMIXED
  vektor v;
  cc = 1. - timestep/ tauber * ( ( 2.0 * bermixed_tot_kin_energy / bermixed_nactive+8.6174101569719990e-06) / (temperature+8.6174101569719990e-06) - 1. );
  bermixed_tot_kin_energy = 0.0;

  for (k=0; k<vtypes; k++){
    bermixed_avg_impuls[k].x = 0;
    bermixed_avg_impuls[k].y = 0;
    bermixed_avg_impuls[k].z = 0;
    bermixed_therm_energy[k] = 0.;
  }
#else
...
#ifdef VIRTUAL_ATOMS
	  cc_ph = 1. - timestep/ tauber_placeholders * ( ( 2.0 * tot_kin_energy_placeholders / nactive_placeholders+8.6174101569719990e-06) / (temperature_placeholders+8.6174101569719990e-06) - 1. );
      if (cc_ph < 0.5) cc_ph = 0.5;
      else if (cc_ph > 2.0) cc_ph = 2.0;
      cc_ph=sqrt(cc_ph);
#endif
...
#ifdef VIRTUAL_ATOMS
  if ((ensemble==ENS_NVE) || (ensemble==ENS_GLOK)) tot_kin_energy_placeholders = 0.0;
#endif
...
#ifdef VIRTUAL_ATOMS
    real kin_energie_1_virt, kin_energie_2_virt;
#endif
#ifdef UNIAX    
...
#pragma vdir vector,nodep
...
#ifdef VIRTUAL_ATOMS
      if (!real_atom[VSORTE(p,i)])
          tot_kin_energy_placeholders += (kin_energie_1 + kin_energie_2) / (4 * MASSE(p,i));
      else
#endif
      tot_kin_energy += (kin_energie_1 + kin_energie_2) / (4 * MASSE(p,i));
#ifdef BERMIXED
      if (bermixed_active[sort] == 1)
    	  bermixed_tot_kin_energy += (kin_energie_1 + kin_energie_2) / (4 * MASSE(p,i));
#endif
#endif
#ifdef UNIAX
...
#ifdef BERMIXED
	  bermixed_avg_impuls[sort].x += IMPULS(p,i,X);
	  bermixed_avg_impuls[sort].y += IMPULS(p,i,Y);
	  bermixed_avg_impuls[sort].z += IMPULS(p,i,Z);
#endif
...
#ifdef VIRTUAL_ATOMS
	  if(!real_atom[VSORTE(p,i)])
	  	tmp *= (steps%update_step==0)?update_step:0;
#endif
  ...
  { /* add up results from different CPUs */
    int nc = 0;
#ifdef BERMIXED
   double *tmpvec1 = (double*)malloc((vtypes*3+8)*sizeof(double));
   double *tmpvec2 = (double*)malloc((vtypes*3+8)*sizeof(double));
#else
...
#endif
#ifdef BERMIXED
    tmpvec1[nc++] = bermixed_tot_kin_energy;
    for (k=0; k<vtypes;k++){
       tmpvec1[nc++] = bermixed_avg_impuls[k].x;
       tmpvec1[nc++] = bermixed_avg_impuls[k].y;
       tmpvec1[nc++] = bermixed_avg_impuls[k].z;
     }
#endif
...
#ifdef VIRTUAL_ATOMS
    tmpvec1[0] = tot_kin_energy_placeholders;
    MPI_Allreduce( tmpvec1, tmpvec2, 1, REAL, MPI_SUM, cpugrid);
    tot_kin_energy_placeholders=tmpvec2[0];
#endif
...
#endif
#ifdef BERMIXED
    bermixed_tot_kin_energy = tmpvec2[nc++];
    for (k=0; k<vtypes;k++){
       bermixed_avg_impuls[k].x = tmpvec2[nc++];
       bermixed_avg_impuls[k].y = tmpvec2[nc++];
       bermixed_avg_impuls[k].z = tmpvec2[nc++];
     }
    free(tmpvec1);
    free(tmpvec2);
#endif
  ...
  for (k=0; k<vtypes; k++){
	  if (bermixed_dof[k] > 0){
		bermixed_avg_impuls[k].x /= num_vsort[k];
		bermixed_avg_impuls[k].y /= num_vsort[k];
		bermixed_avg_impuls[k].z /= num_vsort[k];
  	  }
  }
  ...
  for (k=0; k<NCELLS; ++k) { /* loop over all cells */
     ...
     for (i=0; i<p->n; ++i) { /* loop over all atoms in the cell */
    	 ...
    	 v.x = IMPULS(p,i,X) - bermixed_avg_impuls[sort].x;
    	 v.y = IMPULS(p,i,Y) - bermixed_avg_impuls[sort].y;
    	 v.z = IMPULS(p,i,Z) - bermixed_avg_impuls[sort].z;
    	 bermixed_therm_energy[sort] += (v.x*v.x + v.y*v.y + v.z*v.z)/(2.*MASSE(p,i));
     }
  }
#ifdef MPI
  if ((eng_int  > 0) && (0 == steps % eng_int )){
	  double *tmp1 = (double*)malloc(vtypes*sizeof(real));

	  MPI_Allreduce( bermixed_therm_energy, tmp1, vtypes, REAL, MPI_SUM, cpugrid);
	  for (k=0; k<vtypes; k++){
		bermixed_therm_energy[k] = tmp1[k];
	  }
	  free(tmp1);
  }
#endif
...
#ifdef VIRTUAL_ATOMS
      if (real_atom[VSORTE(p,i)])
          tot_kin_energy_placeholders += (kin_energie_1 + kin_energie_2) / (4.0 * MASSE(p,i));
      else
#endif
...
#ifdef VIRTUAL_ATOMS
  int  p_isReal;
  real E_kin_1_virt = 0.0 ,E_kin_2_virt = 0.0;
  real reibung_ph, eins_d_reib_ph;
  int do_step=steps%update_step;

  if(steps%update_step==0){
    tot_kin_energy_placeholders =0.0;

    reibung_ph         =        1.0 - eta_ph * (timestep*update_step) / 2.0;
    eins_d_reib_ph     = 1.0 / (1.0 + eta_ph * (timestep*update_step) / 2.0);
  }
#endif
...
#ifdef VIRTUAL_ATOMS
        int p_real= real_atom[VSORTE(p,i)]; // 1-real ; 0-virtual
        if (!p_real && do_step!=0) {
      	  continue;
        }
        if (!p_real)
        	 E_kin_1_virt += SPRODN(IMPULS,p,i,IMPULS,p,i) / MASSE(p,i);
        else
#endif
...
#ifdef VIRTUAL_ATOMS
	if(!p_real) {
		IMPULS(p,i,X) = (IMPULS(p,i,X) * reibung_ph + timestep*update_step * KRAFT(p,i,X))
	                           * eins_d_reib_ph * (restrictions + sort)->x;
	    IMPULS(p,i,Y) = (IMPULS(p,i,Y) * reibung_ph + timestep*update_step * KRAFT(p,i,Y))
	                           * eins_d_reib_ph * (restrictions + sort)->y;
	    IMPULS(p,i,Z) = (IMPULS(p,i,Z) * reibung_ph + timestep*update_step * KRAFT(p,i,Z))
	                           * eins_d_reib_ph * (restrictions + sort)->z;
	} else
#endif
    {
...
#ifdef VIRTUAL_ATOMS
        if (!p_real)
             E_kin_2_virt += SPRODN(IMPULS,p,i,IMPULS,p,i) / MASSE(p,i);
        else  	 
#endif
...
#if VIRTUAL_ATOMS
  tot_kin_energy_placeholders = ( E_kin_1_virt + E_kin_2_virt ) / 4.0;
#endif
...
#ifdef VIRTUAL_ATOMS
  if(steps%update_step==0){
  real ttt_ph  = nactive_placeholders * temperature_placeholders;
  eta_ph += timestep * (E_kin_2_virt / ttt_ph - 1.0) * isq_tau_eta_ph;
  }
#endif
