  ...
  if(ep_key == 0) {   /* default: original harmonic potential */
	...
	for (n=0; n<ep_n; ++n) {
	  ...
	  if (n<ep_nind) {
		...
		/* Store directional forces */
		ep_fext_dir[n].x += f * d.x;
		ep_fext_dir[n].y += f * d.y;
		ep_fext_dir[n].z += f * d.z;
	  ...
	  for (n=0; n<ep_n; ++n) {
	      ...
	      if ( dd < ep_rcut*ep_rcut)
		{
	      ...
	      /* old version of extpot, kept for downwards compatibility */
			...
			/* Store directional forces */
			ep_fext_dir[n].x -= ee * d.x;
			ep_fext_dir[n].y -= ee * d.y;
			ep_fext_dir[n].z -= ee * d.z;
	...
	for (i=0; i<p->n; ++i) {
      ...
      totaddforce.z = tmpvec2[2];      
		...
		for (n = 0; n < ep_n; ++n) {
			tmpvec1[0] = ep_fext_dir[n].x;
			tmpvec1[1] = ep_fext_dir[n].y;
			tmpvec1[2] = ep_fext_dir[n].z;
			MPI_Allreduce( tmpvec1, tmpvec2, 4, REAL, MPI_SUM, cpugrid);
			ep_fext_dir[n].x = tmpvec2[0];
			ep_fext_dir[n].y = tmpvec2[1];
			ep_fext_dir[n].z = tmpvec2[2];
		}
#endif
      /* no need for a wall as the total additional impuls is substracted */
