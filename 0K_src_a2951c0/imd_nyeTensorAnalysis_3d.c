...
void init_NyeTensor(){
	...
	if (ada_nbr_r2cut == 0.) {
		...
		} else if (ada_crystal_structure == ADA_BCC_CONFIG || ada_crystal_structure == ADA_FEC_CONFIG || ada_crystal_structure == ADA_NITI_CONFIG) {
	...
	} else if (ada_crystal_structure == ADA_BCC_CONFIG || ada_crystal_structure == ADA_NITI_CONFIG || ada_crystal_structure == ADA_FEC_CONFIG){
	...
	if (myid == 0){
		if (ada_crystal_structure == ADA_FCC_CONFIG) {
			...
			printf("NYETENSOR: B2-NITI analysis scheme selected \n");
		} else if (ada_crystal_structure == ADA_FEC_CONFIG) {
			printf("NYETENSOR: FeC analysis scheme selected \n");
		}
		...
		for (i = 0; i < p->n; i++) {
			if ( ADATYPE(p, i) != ada_default_type &&
				  ( (ada_crystal_structure == ADA_FCC_CONFIG && ADATYPE(p, i)>= 3 && ADATYPE(p,i)<=5) ||
					...
					(ada_crystal_structure == ADA_NITI_CONFIG && ADATYPE(p,i)!=6) ||
					(ada_crystal_structure == ADA_FEC_CONFIG && ADATYPE(p,i)<6)) ) {
