...
#ifdef MONTECARLO
void mc_blackbox(void); // IMD-MONTECARLO Interface
#endif

#ifdef BOPFOX
void init_bopfox();
void calc_forces_bopfox(int);
void pack_bopfox(void);
void unpack_bopfox(void);
void clear_forces(void);
void exit_bopfox(void);

void boplib_init_parameters_(double* boxsize,
		double* rcut, double* r2cut,
		double* rskin, double* rthickskin,
		int* cpuDim, int* pbc, MPI_Comm* comm, int* errorFlag);
void boplib_exit_(void);
void boplib_energy_force_(int* nAtoms, double* positions,
			int* types, double* returnedEnergies, double* returnedForces, int* errorFlag);
void boplib_set_type_(int* index, char* name, size_t* nameLength);
...
/*Slip vector analysis imd_svct.c */
#ifdef SVCT
void init_svct(void);
void do_svct(void);
