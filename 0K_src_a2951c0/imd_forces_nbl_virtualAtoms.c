...
#pragma ivdep
        ...
        for (j=jstart; j<q->n; j++) {
          ...
          s2 = SORTE(q,j);
          r2  = SPROD(d,d);
          if (r2 < cutoffs_r2[s1 * ntypes + s2]) tn++;
        }
      }
    }
  }
...
{
  static int at_max=0, pa_max=0, ncell_max=0, ninnercells_max=0;
  int  c, i, k, n, tn, at, cc, s1;
...
#ifdef OMP
  if (ncells2 > ninnercells_max) {
	  inner_cell_offset = (int *) realloc( inner_cell_offset, ncells2 * sizeof(int) );
	  if (inner_cell_offset==NULL) error("cannot allocate neighbor table");
	  ninnercells_max = ncells2;
  }

  at = 0;
  for (k = 0; k < ncells2; k++) {
	cell *p = cell_array + cnbrs[k].np;
	inner_cell_offset[k] = at;
	at += p->n;
  }
#endif
...
#pragma ivdep,swp
  ...
  n=0;
#ifdef OMP
#pragma omp parallel reduction(+:tot_pot_energy,virial,vir_xx,vir_yy,vir_zz,vir_yz,vir_zx,vir_xy, is_short)
 {
  int n,i,k;
  int num_cellsPerXLayer = (cell_dim.y-2) * (cell_dim.z-2);
  int threadID = omp_get_thread_num();
  int numThreads = omp_get_num_threads();
  int iter;

  for (iter = 0; iter<2;iter++){
	k = num_cellsPerXLayer * (threadID * 2 + iter);
	if (k<ncells) n = inner_cell_offset[k];
    while (k<ncells) {
#else
  for (k=0; k<ncells; k++) {
#endif
...
#ifdef VIRTUAL_ATOMS
      int p_isReal= real_atom[VSORTE(p,i)]; // 1-real ; 0-virtual
      if ( !p_isReal && ignore_non_real_step) {
    	  n++;
    	  continue;
      }
#endif
...
#ifdef VIRTUAL_ATOMS
        int q_isReal = real_atom[VSORTE(q,j)]; // 1-real ; 0-virtual
        if (!q_isReal && ignore_non_real_step) continue;
        int p_addResults = p_isReal || !q_isReal;
        int q_addResults = q_isReal || !p_isReal;
#endif
...
#endif
	{
...
#ifdef VIRTUAL_ATOMS
      KRAFT(q,j,X) -= force.x*p_addResults;
      KRAFT(q,j,Y) -= force.y*p_addResults;
#ifndef TWOD
      KRAFT(q,j,Z) -= force.z*p_addResults;
#endif
      ff.x         += force.x*q_addResults;
      ff.y         += force.y*q_addResults;
#ifndef TWOD
      ff.z         += force.z*q_addResults;
#endif
#else
...
#endif
     {
...
#endif
#ifdef VIRTUAL_ATOMS
            pp.xx             -= d.x * force.x * q_addResults;
            PRESSTENS(q,j,xx) -= d.x * force.x * p_addResults;
            pp.yy             -= d.y * force.y * q_addResults;
            PRESSTENS(q,j,yy) -= d.y * force.y * p_addResults;
            pp.xy             -= d.x * force.y * q_addResults;
            PRESSTENS(q,j,xy) -= d.x * force.y * p_addResults;
            pp.zz             -= d.z * force.z * q_addResults;
            PRESSTENS(q,j,zz) -= d.z * force.z * p_addResults;
            pp.yz             -= d.y * force.z * q_addResults;
            PRESSTENS(q,j,yz) -= d.y * force.z * p_addResults;
            pp.zx             -= d.z * force.x * q_addResults;
            PRESSTENS(q,j,zx) -= d.z * force.x * p_addResults;
        ...
        if (it==jt) {
          if (r2 < rho_h_tab.end[col]) {
#ifdef VIRTUAL_ATOMS
        	  EAM_RHO(q,j) += rho_h * p_addResults;
#else
        	  EAM_RHO(q,j) += rho_h;
#endif
#ifdef EEAM
...
#ifdef OMP
    k++;
    if (k%num_cellsPerXLayer == 0){
    	k += num_cellsPerXLayer * ((numThreads-1) * 2 + 1);
       if (k<ncells) n = inner_cell_offset[k];
    }
  }
 #pragma omp barrier
...
#pragma ivdep,swp
    ...
    for (i=0; i<j; i++) {
#ifdef VIRTUAL_ATOMS
     if(real_atom[VSORTE(p,i)]==0 && ignore_non_real_step) continue;
#endif
...
#pragma omp parallel reduction(+:tot_pot_energy,virial,vir_xx,vir_yy,vir_zz,vir_yz,vir_zx,vir_xy, is_short)
 {
  ...
  int iter;
  /* iter = 0: even layers, iter 1: odd layers*/
  for (iter = 0; iter<2;iter++){
	k = num_cellsPerXLayer * (threadID * 2 + iter);
	if (k<ncells) n = inner_cell_offset[k];
    while (k<ncells) {
...
#pragma ivdep,swp
      ...
      for (m=tl[n]; m<tl[n+1]; m++) {
        ...
        real   r2;
        int    c, j, jt, col1, col2, have_force=0;
        cell   *q;
...
#ifdef VIRTUAL_ATOMS
        //In case of a real-virtual interaction the derivative of the real part
        //is a constant zero and does not need to be computed
        if (q_isReal ^ p_isReal){
        	int col;
        	if(!q_isReal) col = col1;
        	else col = col2;

            if ((r2 < rho_h_tab.end[col])) {
              real grad, rho_strich, rho_i_strich, rho_j_strich, rho_i;

              DERIV_FUNC(rho_strich, rho_h_tab, col, inc, r2, is_short);

          	  if(p_isReal){
          		rho_i_strich = rho_strich;
          		rho_j_strich = 0.;
          	  } else {
          		rho_i_strich = 0.;
          		rho_j_strich = rho_strich;
          	  }
...
#ifdef _OPENMP
#pragma omp parallel for schedule(runtime) reduction(+:tot_sm_es_energy)
#endif
  for (k=0; k<ncells; k++) {
    cell *p = CELLPTR(k);
    for (i=0; i<p->n; i++) {
...
/* 		   DP_E_STAT(p,i,X), DP_E_STAT(p,i,Y),DP_E_STAT(p,i,Z)); */
  ...
  /* Sum up the potential energy of all atoms*/
  tot_pot_energy =0.0;
  for (k=0; k<ncells; k++) {
        cell *p = CELLPTR(k);
        for (i=0; i<p->n; i++){
          ...
          if (real_atom[VSORTE(p,i)])
              ...
              tot_pot_energy_placeholders+=POTENG(p,i);
#else
      	  tot_pot_energy+=POTENG(p,i);
#endif
        }
...
#ifdef VIRTUAL_ATOMS
	if (!ignore_non_real_step) {
		/* sum up results of different CPUs */
		tmpvec1[0] = tot_pot_energy_placeholders;
