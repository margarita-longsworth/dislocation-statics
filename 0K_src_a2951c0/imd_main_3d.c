...
#ifdef STRESSAVERAGE
	if (steps % stressAverage_int > stressAverage_int - stressAverage_delta || steps % stressAverage_int == 0){
		do_press_calc = 1;
	}
#ifdef RELAX
    /* If stresses are to be written into .chkpt & .ss files, the pressure tensor must be available any time*/
...
/* #ifdef BBOOST
    ...
    if ((force_int > 0) && (0 == steps % force_int) && !(checkpt_int == force_int && force_all))
  ...
  last_PxF=PxF;
	...
	if (steps % stressAverage_int > stressAverage_int - stressAverage_delta || steps % stressAverage_int == 0)
		addPressTensor();
	if (stressAverage_int > 0 && 0 == steps % stressAverage_int){
		calculateVolumePerAtom();
		if (checkpt_int !=stressAverage_int || !fullAtomicResolution)
			write_config_select(steps/stressAverage_int, "stress",
					write_atoms_stress, write_header_stress);
	}
#endif
...
#ifdef VIRTUAL_ATOMS
  /*Control temperature of virtual atoms individually*/
  static int updateDTempPh = 0;
  if (steps==steps_min) updateDTempPh = 1;	//Flag that the temperature needs to be update
  	  	  	  	  	  	  	  	  	  	  	//in the next step where placeholder interactions are computed
  if (steps%update_step==0){
    static real dtemp_ph = 0.0;
    if (updateDTempPh) {
      if (use_curr_temp_placeholders==1) {
        temperature_placeholders = 2.0 * tot_kin_energy_placeholders / nactive_placeholders ;
        use_curr_temp_placeholders = 0;
      }
      dtemp_ph = (end_temp - temperature_placeholders) / (steps_max - steps_min);
      updateDTempPh = 0;
    }
    if (steps%update_step==0)
    	temperature_placeholders += dtemp_ph * update_step;
  }
#endif
}
...
{ 
  ...
  if (!do_press_calc) return;//error("pressure tensor incomplete in calculation of total presstens");
