...
{
  int i;
  /*Initialize the table holding the individual cutoffs for each type
   * of interaction */
  cutoffs_r2 = malloc (ntypes*ntypes * sizeof(*cutoffs_r2));
  if (cutoffs_r2 == NULL)
	  error("Cannot allocate info cutoff_r2");
...
void read_pot_table( pot_table_t *pt, char *filename, int ncols, int radial )
{
  ...
  MPI_Bcast( cutoffs_r2, ncols   , REAL, 0, MPI_COMM_WORLD);
#endif
                     ...
                     FILE *infile, int radial)
{
  ...
  for (i=0; i<ncols; ++i) {
    ...
    if (radial) {
    	...
    	cutoffs_r2[i] = MAX(pt->end[i], cutoffs_r2[i]);
    }
  }
...
{
#ifdef LJ84
  /*Lennard Jones type potential with powers 8-4 instead of 12-6*/
  real sig_d_rad2, sig_d_rad4, sig_d_rad8;

  sig_d_rad2 = lj_sigma[p_typ][q_typ] * lj_sigma[p_typ][q_typ] / r2;
  sig_d_rad4 = sig_d_rad2 * sig_d_rad2;
  sig_d_rad8 = sig_d_rad4 * sig_d_rad4;

  *pot = lj_epsilon[p_typ][q_typ] * ( sig_d_rad8 - 2.0 * sig_d_rad4 );
  /* return (1/r)*dV/dr as derivative */
  *grad = -8.0 * lj_epsilon[p_typ][q_typ] / r2 * ( sig_d_rad8 - sig_d_rad4 );
#else
