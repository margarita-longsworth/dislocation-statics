...
void write_header_ada(FILE *out) {
...
#ifdef SVCT
	fprintf(out, " svct_x svct_y svct_z");
#endif
	/* contents line */
#ifdef NYETENSOR
	...
	fprintf(out, "##META\n");
	for (i = 0; i < ep_n; ++i){
		if (ep_key == 1) fprintf(out, "##extpot %i %.4f %.4f %.4f %.4f\n", i, ep_pos[i].x, ep_pos[i].y, ep_pos[i].z, ep_rcut);
		else if (ep_key == 0 && ep_n-ep_nind > 0) fprintf(out, "##wall %i %.4f %.4f\n", i, ep_pos[i].z, ep_rcut);
...
void write_atoms_ada(FILE *out) {
	...
	for (k = 0; k < NCELLS; k++) {
		cell *p = CELLPTR(k);
		for (i = 0; i < p->n; i++) {
			/*if (HOPSTODEFECT(p,i) < 127 || ADATYPE(p,i) == 6 && VSORTE(p,i)<2) {*/
			/*if ( ((ADATYPE(p,i) != ada_default_type && HOPSTODEFECT(p,i) < 127) || ADATYPE(p,i) == 6) && VSORTE(p,i)<2) {*/
			if (ADATYPE(p,i) != ada_default_type) {
...
#ifdef SVCT
					data[n++].f = (float) SLIPVECTOR(p, i, X);
					data[n++].f = (float) SLIPVECTOR(p, i, Y);
					data[n++].f = (float) SLIPVECTOR(p, i, Z);
#endif
#ifdef NYETENSOR
					/*Packed format
				...
				/* ASCII output */
					...
					len += sprintf(outbuf + len," %12f %12f %12f",
							SLIPVECTOR(p, i, X), SLIPVECTOR(p, i, Y), SLIPVECTOR(p, i, Z));
#endif
#ifdef NYETENSOR
					nyeTensorInfo *info = NYE(p,i);
					if (info != NULL) {
...
void write_eng_file_header()
{
  ...
  if (myid == 0) {
    ...
    /* inclusion for virtual atoms*/
#ifdef VIRTUAL_ATOMS
    fprintf(fl, "Epot_pholders ");
    fprintf(fl," Temp_pholders ");
#endif
#if defined(NVT) || defined(NPT) || defined(STM) 
    fprintf(fl, "eta * tau_eta ");

#ifdef VIRTUAL_ATOMS
    fprintf(fl, "eta_ph * tau_eta_ph ");
#endif

#endif
#ifdef BERMIXED
    for (i=0; i<vtypes;i++){
    	fprintf(fl, "temp_ber%d ",i);
    }
#endif
#ifdef FRAC  
...
void write_eng_file(int steps)
{
...
#ifdef VIRTUAL_ATOMS
  real Temp_placeholders,Epot_placeholders, kin_placeholders;
#endif
 ...
 real Temp_stadium = 0.0;
...
#ifdef VIRTUAL_ATOMS
  Epot_placeholders=  tot_pot_energy_placeholders / patoms;
#endif
...
#ifdef VIRTUAL_ATOMS
    Temp_placeholders = 2.0 * tot_kin_energy_placeholders / nactive_placeholders;
#endif
  ...
  /* inclusion for virtual atoms*/
#ifdef VIRTUAL_ATOMS
  fprintf(eng_file," %e", (double) Epot_placeholders);
  fprintf(eng_file," %e", (double) Temp_placeholders);
#endif
...
#ifdef BERMIXED
  for(i=0; i<vtypes; i++){
	  if (bermixed_dof[i] == 0.) tmp = 0.0;
	  else tmp = 2.0 * bermixed_therm_energy[i] / bermixed_dof[i];
	  fprintf(eng_file," %e",     (double) tmp );
  }
#endif
#ifdef FRAC 
...
void write_fext_header()
{
  ...
  if (myid == 0) {
    ...
    fprintf(out, "#C time");
    for (i=0; i<ep_nind; i++) {
      fprintf(out, " x%d y%d z%d f%d area%d atomsincontact%d f_x%d f_y%d f_z%d", i,i,i,i,i,i,i,i,i);
    }
...
{
  str255 fname;
  real tmp, tmpvec[10],tmpvec2[10], ep_car[ep_nind];
  int i,itmpvec[10];
  static int flush_count=0;
  ...
  for (i = 0; i < ep_n; ++i) {
    tmpvec[0] = ep_fext_dir[i].x;
	tmpvec[1] = ep_fext_dir[i].y;
	tmpvec[2] = ep_fext_dir[i].z;
	MPI_Allreduce( tmpvec, tmpvec2, 3, REAL, MPI_SUM, cpugrid);
	ep_fext_dir[i].x = tmpvec2[0];
	ep_fext_dir[i].y = tmpvec2[1];
	ep_fext_dir[i].z = tmpvec2[2];
  }
  ...
  if (myid == 0) {
    ...
    for (i=0; i<ep_nind; i++) {
      fprintf(ind_file, "%f %f %f %f %f %d %f %f %f", ep_pos[i].x, ep_pos[i].y, ep_pos[i].z, ep_fext[i], ep_car[i], ep_atomsincontact[i],
    		  ep_fext_dir[i].x, ep_fext_dir[i].y, ep_fext_dir[i].z );
    }
    for (i=ep_nind; i<ep_n; i++) {
...
void write_header_config(FILE *out)
{
...
#endif
#ifdef STRESSAVERAGE
  if (checkpt_int == stressAverage_int && fullAtomicResolution)
#ifdef TWOD
	atompar += 4;
#else
    atompar += 7;
  ...
  if (checkpt_int == stressAverage_int && fullAtomicResolution)
#ifdef TWOD
	  fprintf(out, " s_xx s_yy s_xy s_vM" );
#else
  	  fprintf(out, " s_xx s_yy s_zz s_yz s_zx s_xy s_vM" );
#endif
#endif
#ifdef WRITEF
  if (checkpt_int == force_int && force_all)
#ifdef TWOD
	  fprintf(out, " f_x f_y" );
#else
  	  fprintf(out, " f_x f_y f_z" );
#endif
#endif
#ifdef CNA
