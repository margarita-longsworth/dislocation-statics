...
#ifdef VIRTUAL_ATOMS
         real_atom= (int*) malloc(vtypes * sizeof(int));
         if (NULL==real_atom)
           error("Cannot allocate memory for virtual vector\n");
         for (k=0; k<vtypes; k++)
           real_atom[k]=0;  /*   change effected */
#endif /* End part of virtual atoms*/
...
#endif
#ifdef BERMIXED
        bermixed_avg_impuls = (vektor *) malloc(vtypes * sizeof(vektor));
        if (NULL == bermixed_avg_impuls)
           error("Cannot allocate memory for bermixed_avg_impuls\n");
        bermixed_therm_energy = (real *) malloc(vtypes * sizeof(real));
        if (NULL == bermixed_therm_energy)
          error("Cannot allocate memory for bermixed_therm_energy\n");
        bermixed_dof = (long *) malloc(vtypes * sizeof(long));
        if (NULL == bermixed_dof)
          error("Cannot allocate memory for bermixed_dof\n");
        for(k=0; k<vtypes;k++)
          bermixed_dof[k] = 0;
#endif
      }
...
//#ifdef MONTECARLO
//    else if (strcasecmp(token,"r_sample")==0) {
      /* radius for sub spheres to be sampled in MonteCarlo  */
//      getparam("r_sample",&r_sample,PARAM_REAL,1,1);
//    }
//#endif

#ifdef VIRTUAL_ATOMS
       	else if (strcasecmp(token,"realvector")==0) {
    	      if (vtypes==0)
    	        error("specify parameter total_types before realvector");
    	      /* restrictions for virtual atom types */
    	      /* format: type  1 (real atom) : default = 0 (considered as virtual atom) */
    	      getparam(token,&vec,PARAM_INT,2,2);
    	      if (vec.x>vtypes-1)
    	        error("Real_atom no defined for non existing real atom type\n");
    	      val = vec.y;
...
#ifdef VIRTUAL_ATOMS
    else if(strcasecmp(token,"ph_frequency")==0) {
      /* update frequency for placeholder interaction */
      getparam("ph_frequency",&update_step,PARAM_INT,1,1);
    }
    else if (strcasecmp(token,"starttemp_ph")==0) {
      /* temperature at start of placeholdes system */
      getparam("starttemp_ph",&temperature_placeholders,PARAM_REAL,1,1);
    }
    else if (strcasecmp(token,"use_current_temp_ph")==0) {
       /* set imposed temperature to current system temperature (for placeholders system) */
       use_curr_temp_placeholders = 1;
    }
#endif
...
#ifdef VIRTUAL_ATOMS
    else if (strcasecmp(token,"tau_berendsen_ph")==0) {
          /* reading temperature interval for placeholder system */
          getparam("tau_berendsen_ph",&tauber_placeholders,PARAM_REAL,1,1);
        }
#endif
#endif
...
#ifdef VIRTUAL_ATOMS
    else if (strcasecmp(token,"eta_ph")==0) {
      /* eta_ph variable for NVT or NPT thermostat for placeholders */
      getparam("eta_ph",&eta_ph,PARAM_REAL,1,1);
    }
    else if (strcasecmp(token,"tau_eta_ph")==0) {
      /* time constant tau_eta_ph for thermostat for placeholders */
      getparam("tau_eta_ph",&tau_eta_ph,PARAM_REAL,1,1);
      if (tau_eta_ph == (real)0) {
        error("tau_eta_ph for placeholders is zero.\n");
      }
      isq_tau_eta_ph = 1.0 / SQR(tau_eta_ph);
    }
    else if (strcasecmp(token,"isq_tau_eta_ph")==0) {
      /* inverse of square of time constant tau_eta_ph for thermostat (placeholders) */
      getparam("isq_tau_eta_ph",&isq_tau_eta_ph,PARAM_REAL,1,1);
      if (isq_tau_eta_ph == (real)0) tau_eta_ph = 0.0;
      else tau_eta_ph = 1.0 / sqrt(isq_tau_eta_ph);
    }
    else if (strcasecmp(token,"inv_tau_eta_ph")==0) {
      /* inverse of time constant tau_eta_ph for thermostat (placeholders) */
      getparam("inv_tau_eta_ph",&isq_tau_eta_ph,PARAM_REAL,1,1);
      if (isq_tau_eta_ph == (real)0) tau_eta_ph = 0.0;
      else tau_eta_ph = 1.0 / isq_tau_eta_ph;
      isq_tau_eta_ph = SQR(isq_tau_eta_ph);
    }
#endif
	...
	error("Nonsense value for ramp_fraction detected, please check prameter file!");
      ...
      cna_write_statistics = 1;
...
#endif
#if defined(SVCT) || defined(ADA) || defined (STRESSAVERAGE)
    	...
    	} else if (strcasecmp(tmpstr,"niti") == 0) {
    	    ada_crystal_structure = ADA_NITI_CONFIG;
    	} else if (strcasecmp(tmpstr,"fec") == 0) {
    	    ada_crystal_structure = ADA_FEC_CONFIG;
    	}
    }
...
#ifdef STRESSAVERAGE
    else if (strcasecmp(token, "stressAverage_int") == 0) {
      	/* interval between two averages stress files */
      	getparam("stressAverage_int",&stressAverage_int,PARAM_INT,1,1);
    }
    else if (strcasecmp(token, "stressAverage_delta") == 0) {
          	/* average the last #delta steps in stress calculation*/
          	getparam("stressAverage_delta",&stressAverage_delta,PARAM_INT,1,1);
    }
    else if (strcasecmp(token, "fullAtomicResolution") == 0) {
       	/* 1 = write stress for every atom, 0 = write stress in each cell */
       	getparam("fullAtomicResolution",&fullAtomicResolution,PARAM_INT,1,1);
    }
#endif

#ifdef BERMIXED
    else if (strcasecmp(token,"ber_active")==0) {
      if (ntypes==0) error("specify parameter vtypes before ber_active");
      bermixed_active = (int *) malloc( vtypes*sizeof(int));
      if (NULL==bermixed_active) error("cannot allocate bermixed_active");
      getparam(token, bermixed_active, PARAM_INT, vtypes, vtypes);
    }
#endif
#ifdef BOPFOX
        else if (strcasecmp(token,"bopfox_rcut")==0) {
          getparam(token,&bopfox_rcut,PARAM_REAL,1,1);
        }
        else if (strcasecmp(token,"bopfox_r2cut")==0) {
          getparam(token,&bopfox_r2cut,PARAM_REAL,1,1);
        }
        else if (strcasecmp(token,"bopfox_rskin")==0) {
          getparam(token,&bopfox_rskin,PARAM_REAL,1,1);
        }
        else if (strcasecmp(token,"bopfox_rthickskin")==0) {
          getparam(token,&bopfox_rthickskin,PARAM_REAL,1,1);
        }
        else if (strcasecmp(token,"bopfox_type")==0) {
          getparam(token,&bopfoxCurrentType,PARAM_INT,1,1);
        }
        else if (strcasecmp(token,"bopfox_typename")==0) {
          getparam("bopfox_typename", bopfox_typename[bopfoxCurrentType] ,PARAM_STR,1,255);
        }
#endif
#ifdef VISCOUS
...
//  if(r_sample==0){
//	 error("r_sample is missing or zero.");
//  }
//#endif
...
#if defined(FBC) || defined(RIGID) || defined(DEFORM) || defined( VIRTUAL_ATOMS)
  if (vtypes == 0)
    error("FBC, RIGID, VIRTUAL_ATOMS and DEFORM require parameter total_types to be set");
#endif
  if (vtypes == 0) {
    vtypes = ntypes;
    restrictions = (vektor *) malloc( vtypes * sizeof(vektor) );
...
#if defined(PAIR) && !defined(VARCHG) && !defined(TERSOFFMOD) && !defined(BOPFOX)
...
#ifdef VIRTUAL_ATOMS
  if (temperature_placeholders == 0)
    error("starttemp_ph for placeholders  is missing or zero.");
#endif
#endif
...
#endif
#if defined(SVCT) && defined(TWOD)
  error("Option SVCT is not supported in 2D");
#endif

#if defined(ADA) || defined(SVCT) || defined(STRESSAVERAGE)
#ifdef ADA
...
#else
  if (ada_nbr_r2cut == 0.){
	  error("Nearest neighbor cutoff distance \"ada_nbr_rcut\" is missing or zero in the parameter file");
  }
#endif
#endif
...
#ifdef STRESSAVERAGE
  /* stresses are added to chkpt if no interval is given*/
  if (stressAverage_int == 0) stressAverage_int = checkpt_int;
  /* the number of averaging steps cannot exceed the number of steps between output*/
  if (stressAverage_delta > stressAverage_int) stressAverage_delta = stressAverage_int;
  if (stressAverage_delta == 0) stressAverage_delta = 1;
#endif

#ifdef BERMIXED
  if (bermixed_active==NULL) error("ber_active is missing in the parameter file");
  for (k=0; k<vtypes; k++){
	  if (bermixed_active[k] != 1 && bermixed_active[k] != 0)
		  error("ber_active must be either 0 (don't control temperature for type) or 1 (control temp.)");
  }
#endif
#ifdef BOPFOX
...
#ifdef VIRTUAL_ATOMS
  if (NULL==real_atom){
      real_atom= (int*) malloc(vtypes * sizeof(int)); /*included part*/
      if (NULL==real_atom)
        error("Cannot allocate memory for virtual types on client.");
    }
  int avc=1;
  MPI_Bcast( real_atom, vtypes , MPI_INT, 0, MPI_COMM_WORLD); /*included part*/
#endif
...
#ifdef VIRTUAL_ATOMS
  MPI_Bcast( &temperature_placeholders ,   1, REAL,     0, MPI_COMM_WORLD);
  MPI_Bcast( &use_curr_temp_placeholders,  1, MPI_INT,  0, MPI_COMM_WORLD);
  MPI_Bcast(&update_step,  1, MPI_INT,  0, MPI_COMM_WORLD);
#endif
...
#ifdef VIRTUAL_ATOMS
  MPI_Bcast( &tauber_placeholders, 1, REAL, 0, MPI_COMM_WORLD); // broadcasting controlling parameter for placeholder system
#endif
#endif
#ifdef VISCOUS
...
#ifdef VIRTUAL_ATOMS
  MPI_Bcast( &eta_ph ,         1 , REAL, 0, MPI_COMM_WORLD);
  MPI_Bcast( &isq_tau_eta_ph , 1 , REAL, 0, MPI_COMM_WORLD);
...
#endif
#ifdef STRESSAVERAGE
  MPI_Bcast( &stressAverage_int,  1, MPI_INT, 0, MPI_COMM_WORLD);
  MPI_Bcast( &stressAverage_delta,  1, MPI_INT, 0, MPI_COMM_WORLD);
  MPI_Bcast( &fullAtomicResolution,  1, MPI_INT, 0, MPI_COMM_WORLD);
#endif
#ifdef BERMIXED
  if (NULL == bermixed_active) {
	  bermixed_active = (int *) malloc(vtypes * sizeof(int));
	  if (NULL == bermixed_active)
		  error("Cannot allocate memory for bermixed_active on client.");
  }
  MPI_Bcast( bermixed_active, vtypes, MPI_INT, 0, MPI_COMM_WORLD);

  bermixed_avg_impuls = (vektor *) malloc(vtypes * sizeof(vektor));
  if (NULL == bermixed_avg_impuls)
    error("Cannot allocate memory for bermixed_avg_impuls on client.");
  bermixed_therm_energy = (real *) malloc(vtypes * sizeof(real));
  if (NULL == bermixed_therm_energy)
    error("Cannot allocate memory for bermixed_therm_energy on client.");
  bermixed_dof = (long *) malloc(vtypes * sizeof(long));
  if (NULL == bermixed_dof)
    error("Cannot allocate memory for bermixed_dof on client.");
   for(k=0; k<vtypes;k++)
	  bermixed_dof[k] = 0;
#endif
#ifdef BOPFOX
   MPI_Bcast( &bopfox_rcut, 1, REAL, 0, MPI_COMM_WORLD);
   MPI_Bcast( &bopfox_r2cut,  1, REAL, 0, MPI_COMM_WORLD);
   MPI_Bcast( &bopfox_rskin,  1, REAL, 0, MPI_COMM_WORLD);
   MPI_Bcast( &bopfox_rthickskin,  1, REAL, 0, MPI_COMM_WORLD);
   for (i=0; i<ntypes; i++)
	   MPI_Bcast( bopfox_typename[i], 255, MPI_CHAR, 0, MPI_COMM_WORLD);
