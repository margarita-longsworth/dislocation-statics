...
//EXTERN int* md_cpu_dim;        // CPU dimension for MC
//EXTERN int* md_restriction;    // restriction/ DOF definition vector
//EXTERN double* md_simbox;     //  Simbox physical dimensions
//EXTERN int md_tot_types INIT(0);      // no of total element types (CHECK for correctness??)
//EXTERN int md_real_types INIT(0);      // no of real element types
...
EXTERN int  compute_energy_between_fixed_atoms INIT(0);

EXTERN int  last_nbl_len   INIT(0);
#endif

#ifdef VIRTUAL_ATOMS
EXTERN long patoms  INIT(0);      /* Total number of placeholders */
EXTERN long nactive_placeholders INIT(0); /* number of transl. degrees of freedom for place holders */
EXTERN int *real_atom INIT(NULL);  /* holds the real atom information */
EXTERN real tot_pot_energy_placeholders INIT(0.0); /* potential energy contribution for virtual atoms*/
EXTERN real tot_kin_energy_placeholders INIT(0.0); /* kinetic energy contribution for virtual atoms*/
EXTERN real pressure_placeholders INIT(0.0);       /* pressure for placeholders */
EXTERN int  update_step INIT(1);                    /* position update frequency for placeholders*/
EXTERN real temperature_placeholders INIT(0.0);   /* temperature for placeholders*/
EXTERN int  use_curr_temp_placeholders INIT(0);   /* current temperature for placeholder*/
#ifdef BER
EXTERN real tauber_placeholders INIT(0);     /* Relaxation constant Berendsen Thermo (for placeholders) */
#endif
#if defined(NVT) || defined(NPT) || defined(STM)
EXTERN real eta_ph INIT(0.0);          /* Nose-Hoover heat bath variable */
EXTERN real tau_eta_ph INIT(0.0);      /* Nose-Hoover heat bath relax. time */
EXTERN real isq_tau_eta_ph INIT(0.0);  /* isq_tau_eta : inverse of tau_eta^2 */
#endif
#endif
...
EXTERN real *cutoffs_r2 INIT(NULL);		/* The squared cutoff radii*/
...
EXTERN int n_out_grps   INIT(0);    /* number of output groups */
#ifdef MPI
...
#endif
#ifdef BERMIXED
EXTERN int *bermixed_active INIT(NULL);   /* enables/disables Berendsen Thermo for specific atom types only */
EXTERN real bermixed_tot_kin_energy INIT(0);
EXTERN long bermixed_nactive INIT(0);

EXTERN vektor *bermixed_avg_impuls INIT(0);		/* Average impuls, thermal energy and */
EXTERN real   *bermixed_therm_energy INIT(0);   /* degrees of freedom for each (virtual) atom type */
EXTERN long   *bermixed_dof INIT(0);
#endif
...
#endif
#ifdef STRESSAVERAGE
EXTERN int stressAverage_int INIT(0);
EXTERN int stressAverage_delta INIT(0);
EXTERN int stressAverage_addedThisStep INIT(0);
EXTERN int stressAverage_stepsAveraged INIT(0);
EXTERN int fullAtomicResolution INIT(1);
#endif
#ifdef NYETENSOR
...
#ifdef BOPFOX
EXTERN int bopfox_nAtoms    INIT(0);
EXTERN int bopfox_nAtomsMax INIT(0);
EXTERN double *bopfoxInputPositions  INIT(NULL);
EXTERN int *bopfoxInputTypes  INIT(NULL);
EXTERN double *bopfoxResultForce INIT(NULL);
EXTERN double *bopfoxResultEnergy INIT(NULL);

EXTERN double bopfox_rcut INIT(0.0);
EXTERN double bopfox_r2cut INIT(0.0);
EXTERN double bopfox_rskin INIT(0.0);
EXTERN double bopfox_rthickskin INIT(0.0);

EXTERN int bopfoxCurrentType INIT(0);
EXTERN str255 bopfox_typename[10];
#endif
