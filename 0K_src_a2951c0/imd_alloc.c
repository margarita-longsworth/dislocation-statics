...
void copy_atom_cell_cell(cell *to, int i, cell *from, int j)
{
...
#endif
#ifdef SVCT
  to->slipVector X(i) = from->slipVector X(j);
  to->slipVector Y(i) = from->slipVector Y(j);
  to->slipVector Z(i) = from->slipVector Z(j);

  to->slipReference X(i) = from->slipReference X(j);
  to->slipReference Y(i) = from->slipReference Y(j);
  to->slipReference Z(i) = from->slipReference Z(j);
#endif
#ifdef STRESSAVERAGE
  to->vol[i] = from->vol[j];
  to->presstensSum[i] = from->presstensSum[j];
#endif
...
#endif /* AVPOS */
#ifdef STRESSAVERAGE
  to->presstensSum[i].xx = from->presstensSum[j].xx;
  to->presstensSum[i].yy = from->presstensSum[j].yy;
  to->presstensSum[i].xy = from->presstensSum[j].xy;
#ifndef TWOD
  to->presstensSum[i].zz = from->presstensSum[j].zz;
  to->presstensSum[i].yz = from->presstensSum[j].yz;
  to->presstensSum[i].zx = from->presstensSum[j].zx;
#endif
#endif /* STRESSAVERAGE */
#endif /* STRESS_TENS */
#ifdef SHOCK
  ...
  to->refEpot[i] = from->refEpot[j];
  to->refRho[i] = from->refRho[j];
...
#endif
#ifdef SVCT
  memalloc( &p->slipVector, n*SDIM, sizeof(real), al, ncopy*SDIM, 0, "slipVector" );
  memalloc( &p->slipReference, n*SDIM, sizeof(real), al, ncopy*SDIM, 0, "slipReference" );
#endif
#ifdef STRESSAVERAGE
  memalloc( &p->vol, n, sizeof(real), al, ncopy, 0, "volume" );
  memalloc( &p->presstensSum, n, sizeof(sym_tensor), al, ncopy, 0, "SumPressTensor" );
#endif
#ifdef NYETENSOR
...
// #endif
  ...
  memalloc( &p->refEpot,  n, sizeof(real), al, ncopy, 0, "refEpot" );
  memalloc( &p->refRho,  n, sizeof(real), al, ncopy, 0, "refRho" );
